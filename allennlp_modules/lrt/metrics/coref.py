...
@Metric.register("detail_coref")
class DetailCoref(Metric):
    ...
    def get_metric(self, reset: bool = False):
        metrics = ['muc', 'b_cubed', 'ceafe']
        ret = dict()
        for name, scorer in zip(metrics, self.scorers):
            ret[name + '_p'] = scorer.get_precision() * 100
            ret[name + '_r'] = scorer.get_recall() * 100
            ret[name + '_f1'] = scorer.get_f1() * 100
        ...
        ret['coref_p'] = precision * 100
        ret['coref_r'] = recall * 100
        ret['coref_f1'] = f1_score * 100
        if reset:
            self.reset()
        return ret
