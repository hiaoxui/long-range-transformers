...
from ..metrics.coref import DetailCoref
...
@Model.register("coarse2fine")
class Coarse2Fine(Model):
    """
    ...
    def from_params(
        ...
        text_field_embedder: TextFieldEmbedder = pop_and_construct_arg(
            'Coarse2Fine', 'text_field_embedder', TextFieldEmbedder, None, params, **extras
        )
        encoder_dim = text_field_embedder.get_output_dim()
        context_layer: Seq2SeqEncoder = pop_and_construct_arg(
            'Coarse2Fine', 'context_layer', Seq2SeqEncoder, None, params, input_dim=encoder_dim, **extras
        )
        feature_size = params.get('feature_size')
        span_emb_dim = 3 * encoder_dim + feature_size
        mention_feedforward: FeedForward = pop_and_construct_arg(
            'Coarse2Fine', 'mention_feedforward', FeedForward, None, params, input_dim=span_emb_dim, **extras
        )
        span_pair_emb_dim = 3 * span_emb_dim + feature_size
        antecedent_feedforward: FeedForward = pop_and_construct_arg(
            'Coarse2Fine', 'antecedent_feedforward', FeedForward, None, params, input_dim=span_pair_emb_dim, **extras
        )
    ...
    def __init__(
        ...
        # transformer_model_name = kwargs.pop("transformer_model_name")
        # use_context = kwargs.pop("use_context")
        # segment_length = kwargs.pop("segment_length")
        # context_length = kwargs.pop("context_length")
        super().__init__(vocab, **kwargs)

        # self._text_field_embedder = Seq2Seq.get_enecoder_model(transformer_model_name,
        #                                                        use_context=True if use_context == 1 else False,
        ...
        self._mention_scorer = TimeDistributed(
            nn.Linear(mention_feedforward.get_output_dim(), 1)
        )
        self._antecedent_feedforward = TimeDistributed(antecedent_feedforward)
        self._antecedent_scorer = TimeDistributed(
            nn.Linear(antecedent_feedforward.get_output_dim(), 1)
        )
        ...
        if self._coarse_to_fine:
            self._coarse2fine_scorer = nn.Linear(
        ...
        self._conll_coref_scores = DetailCoref()
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        metrics = self._conll_coref_scores.get_metric(reset)
        metrics['mention_recall'] = self._mention_recall.get_metric(reset)

        if not reset:
            return {
                "coref_f1": int(metrics['coref_f1'] * 100) / 100,
            }
        return metrics
