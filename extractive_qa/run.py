...
from transformers import AutoTokenizer, AutoModel, AutoConfig, AutoModelWithLMHead
...
# sfrom pytorch_lightning.overrides.data_parallel import LightningDistributedDataParallel
import os
import sys
...
class TriviaQADataset(Dataset):
    ...
    def __init__(self, file_path, tokenizer, max_seq_len, max_doc_len, doc_stride,
        ...
        with open(self.file_path, "r", encoding='utf-8') as f:
            ...
            self.data_json = json.load(f)['data']  #:1000]
...
class TriviaQA(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        self.model = Seq2Seq.get_enecoder_model(args.model_name,
    ...
    def forward(self, input_ids, attention_mask, segment_ids, start_positions, end_positions, answer_token_ids):
        # elf._set_static_graph()
        if "longformer" in self.args.model_name and self.args.use_global == 1:
            ...
            global_attention_mask[:, :question_end_index.item()] = 1
        else:
    ...
    def validation_step(self, batch, batch_nb):
        ...
        if self.args.seq2seq:
            ...
            em_score = evaluation_utils.metric_max_over_ground_truths(evaluation_utils.exact_match_score, answer_text,
                                                                      aliases)
        ...
        return_res = {'vloss': loss, 'vem': exact_match.mean(),
    ...
    def _get_question_end_index(self, input_ids):
        eos_token_indices = (input_ids == self.tokenizer.sep_token_id).nonzero()
        assert eos_token_indices.ndim == 2
        # rint(eos_token_indices.size())
    ...
    def train_dataloader(self):
        ...
        sampler = torch.utils.data.distributed.DistributedSampler(dataset,
                                                                  shuffle=True)  # if self.trainer.use_ddp else None
    ...
    def val_dataloader(self):
        ...
        print(len(dataset))
        sampler = torch.utils.data.distributed.DistributedSampler(dataset,
                                                                  shuffle=False)  # if self.trainer.use_ddp else None
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument("--use-global", required=True, type=int)
        return parser
...
def main(args):
    ...
    if os.path.exists(args.save_prefix + "/ckpt.ckpt"):
        trainer = pl.Trainer(default_root_dir=args.save_prefix, gpus=args.gpus, strategy=CustomDDPPlugin(),
                             val_check_interval=0.5,
                             resume_from_checkpoint=args.save_prefix + "/ckpt.ckpt")  # 000)#log_every_n_steps=50)
    else:
        trainer = pl.Trainer(default_root_dir=args.save_prefix, gpus=args.gpus, strategy=CustomDDPPlugin(),
                             val_check_interval=0.5)
