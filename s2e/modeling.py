...
from lre import LongRangeEmbedder
...
class FullyConnectedLayer(Module):
    def __init__(self, config, input_dim, output_dim, dropout_prob):
        ...
        if config.model_type == 'xlnet':
            self.activation_func = ACT2FN[config.ff_activation]
        else:
...
class S2E(BertPreTrainedModel):
    def __init__(self, args):
        encoder = LongRangeEmbedder(
            args.m, max_length=args.segment, use_global=args.use_global, gradient_checkpointing=args.checkpoint,
            attn_args={'type': 'huggingface' if not args.performer else 'performer'},
            lstm=args.lstm, overlapping=args.overlap
            # load_weights=False if args.no_load else True
            # We no longer do this experiment
        )
    ...
    def forward(self, input_ids, attention_mask=None, gold_clusters=None, return_all_outputs=False):
        ...
        if self.config.model_type == 'xlnet':
            input_ids[input_ids == 49518] = 31997
        sequence_output = self.encoder(token_ids=input_ids, mask=attention_mask) # [batch_size, seq_len, dim]
