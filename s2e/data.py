...
class CorefDataset(Dataset):
        ...
        for doc_key, words, clusters, speakers in examples:
            ...
            for idx, (word, speaker) in enumerate(zip(words, speakers)):
                if last_speaker != speaker:
                    speaker_prefix = [consts.SPEAKER_START] + self.tokenizer.encode(" " + speaker,
                                                                             add_special_tokens=False) + [consts.SPEAKER_END]
    ...
    def pad_clusters_inside(self, clusters):
        return [cluster + [(consts.NULL_ID_FOR_COREF, consts.NULL_ID_FOR_COREF)] * (self.max_cluster_size - len(cluster)) for cluster
                in clusters]
    ...
    def pad_batch(self, batch, max_length):
        ...
        for example in batch:
            # tokens = self.tokenizer.convert_ids_to_tokens(example[0], )
