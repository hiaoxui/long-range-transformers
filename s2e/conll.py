    ...
    for doc_key, clusters in predictions.items():
        ...
        for cluster_id, mentions in enumerate(clusters):
            for start, end in mentions:
                if start == len(subtoken_map[doc_key]):
                    logger.warning('start reached the end. minus 1.')
                    start -= 1
                if end == len(subtoken_map[doc_key]):
                    logger.warning('end reached the end. minus 1.')
                    end -= 1
